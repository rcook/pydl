...
- **Verification.** `release.yaml` writes a `SHA256SUMS` manifest alongside the per-target archives. `self-update` fetches it and hashes the downloaded archive against the matching entry before extracting. A missing manifest is a hard error: pass `--allow-missing-checksum` to opt out for the rare case of self-updating from a pre-v0.1.5 release that doesn't publish `SHA256SUMS`. The flip from "warn and proceed" to "refuse" happened once five consecutive releases (v0.1.5 onward) had shipped manifests cleanly.
